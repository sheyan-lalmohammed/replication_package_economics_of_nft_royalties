...
# ----------------------------
# CONFIG  (exactly from estimate_paper_match_v2.py)
# ----------------------------
PLATFORM_B = "Blur"
PLATFORM_O = "OpenSea"
...
def parse_args():
    ...
    p.add_argument("--outdir", type=str, default="paper_match_estimates_v2_bootstrap_exact")
    p.add_argument("--n_boot", type=int, default=200)
    p.add_argument("--seed", type=int, default=123)
    p.add_argument(
        "--bootstrap_unit",
        type=str,
        choices=["market", "collection"],
        default="collection",
        help="Resample collection-week markets or full collections with replacement.",
    )
    p.add_argument(
        "--save_each_boot_fitted",
        action="store_true",
        help="If set, save fitted_markets for each successful bootstrap draw.",
    )
    return p.parse_args()


# ----------------------------
# HELPERS  (exactly from estimate_paper_match_v2.py)
# ----------------------------
...
# ----------------------------
# DATA PREP  (exactly from estimate_paper_match_v2.py)
# ----------------------------
...
# ----------------------------
# MODEL  (exactly from estimate_paper_match_v2.py)
# ----------------------------
@dataclass
class Params:
    delta_B: float
...
def predict_shares(wide: pd.DataFrame, par: Params) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # exact: NO clipping at zero here
...
# ----------------------------
# MOMENTS  (exactly from estimate_paper_match_v2.py)
# ----------------------------
...
# ----------------------------
# ESTIMATION WRAPPER  (exact optimizer settings)
# ----------------------------
def estimate_once(wide: pd.DataFrame):
...
# BOOTSTRAP SAMPLING
# ----------------------------
def bootstrap_sample_wide(wide: pd.DataFrame, rng: np.random.Generator, unit: str) -> pd.DataFrame:
    if unit == "market":
        draw = rng.choice(np.arange(len(wide)), size=len(wide), replace=True)
        out = wide.iloc[draw].copy().reset_index(drop=True)
        out["boot_row_id"] = np.arange(len(out))
        return out

    if unit == "collection":
        collections = wide["collection_address"].astype(str).unique()
        draw = rng.choice(collections, size=len(collections), replace=True)

        pieces = []
        for i, c in enumerate(draw):
            g = wide[wide["collection_address"].astype(str) == str(c)].copy()
            g["boot_cluster_id"] = i
            pieces.append(g)
    ...
    raise ValueError(f"Unknown bootstrap_unit: {unit}")
...
def main():
    ...
    print("Loading original sample...")
    wide = load_and_pivot(args.input_csv)

    print("Estimating baseline model...")
    base_res, base_par = estimate_once(wide)
    base_sB, base_switch = predict_shares(wide, base_par)

    base_fitted = wide.copy()
    base_fitted["share_B_hat"] = base_sB
    base_fitted["switch_hat"] = base_switch
    base_fitted.to_csv(os.path.join(args.outdir, "baseline_fitted_markets.csv"), index=False)

    baseline_params = pd.DataFrame([{
        "delta_B": base_par.delta_B,
        "lambda_S": base_par.lambda_S,
        "phi_K": base_par.phi_K,
        "k0": base_par.k0,
        "k1": base_par.k1,
        "gamma_S": base_par.gamma_S,
        "objective": float(base_res.fun),
        "success": bool(base_res.success),
        "message": str(base_res.message),
        "n_markets": int(len(wide)),
    }])
    baseline_params.to_csv(os.path.join(args.outdir, "baseline_params.csv"), index=False)
    ...
    print(f"\nRunning {args.n_boot} bootstrap replications using {args.bootstrap_unit} bootstrap...")
    for b in range(args.n_boot):
        try:
            boot_wide = bootstrap_sample_wide(wide, rng, args.bootstrap_unit)
            res_b, par_b = estimate_once(boot_wide)

            row = {
                "boot_id": b + 1,
                "success": bool(res_b.success),
                "message": str(res_b.message),
                "objective": float(res_b.fun),
                "n_markets": int(len(boot_wide)),
                "delta_B": par_b.delta_B,
                "lambda_S": par_b.lambda_S,
                "phi_K": par_b.phi_K,
                "k0": par_b.k0,
                "k1": par_b.k1,
                "gamma_S": par_b.gamma_S,
            }
            draws.append(row)

            if args.save_each_boot_fitted and res_b.success:
                sB_b, switch_b = predict_shares(boot_wide, par_b)
                out_b = boot_wide.copy()
                out_b["share_B_hat"] = sB_b
                out_b["switch_hat"] = switch_b
                out_b.to_csv(os.path.join(args.outdir, f"boot_fitted_markets_{b+1:04d}.csv"), index=False)

            if (b + 1) % 10 == 0 or (b + 1) == args.n_boot:
                print(f"Completed {b+1}/{args.n_boot}")

        except Exception as e:
            failures.append({"boot_id": b + 1, "error": str(e)})
            print(f"Bootstrap draw {b+1} failed: {e}")

    draws_df = pd.DataFrame(draws)
    draws_df.to_csv(os.path.join(args.outdir, "bootstrap_draws.csv"), index=False)

    if failures:
        pd.DataFrame(failures).to_csv(os.path.join(args.outdir, "bootstrap_failures.csv"), index=False)

    good = draws_df[draws_df["success"] == True].copy()

    params = ["delta_B", "lambda_S", "phi_K", "k0", "k1", "gamma_S"]
    summary_rows = []

    for p in params:
        x = pd.to_numeric(good[p], errors="coerce").dropna().to_numpy()
        if len(x) == 0:
            summary_rows.append({
                "parameter": p,
                "baseline_estimate": float(baseline_params.iloc[0][p]),
                "n_successful_boot": 0,
                "boot_mean": np.nan,
                "boot_sd": np.nan,
                "ci_2_5": np.nan,
        ...
        summary_rows.append({
            "parameter": p,
            "baseline_estimate": float(baseline_params.iloc[0][p]),
            "n_successful_boot": int(len(x)),
            "boot_mean": float(np.mean(x)),
            "boot_sd": float(np.std(x, ddof=1)) if len(x) > 1 else np.nan,
            "ci_2_5": float(np.quantile(x, 0.025)),
            "ci_50": float(np.quantile(x, 0.50)),
            "ci_97_5": float(np.quantile(x, 0.975)),
        })

    ci_df = pd.DataFrame(summary_rows)
    ci_df.to_csv(os.path.join(args.outdir, "bootstrap_confidence_intervals.csv"), index=False)

    meta = {
        "input_csv": args.input_csv,
        "bootstrap_unit": args.bootstrap_unit,
        "n_boot_requested": int(args.n_boot),
        "n_boot_successful": int(len(good)),
        "seed": int(args.seed),
        "estimator_match": "Exact bootstrap wrapper for uploaded estimate_paper_match_v2.py",
    }
    with open(os.path.join(args.outdir, "bootstrap_metadata.json"), "w") as f:
